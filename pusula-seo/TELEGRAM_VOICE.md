...
- **Search Console milestones** (indexed, first impression/click, ranking improvement, top-10 reached, ranking drop — `scripts/telegram/format.mjs`'s `formatGscMilestoneEvent`): these are individual event notifications, not aggregate reports, so they skip the bullets/Top-Actions skeleton entirely — each is already one complete, atomic thing. **Positive ones are wins, say so plainly** ("İlk 10'a Girdi 🔥" isn't a data point, it's good news — the assistant should sound genuinely pleased, not neutral). Ranking drops are the one case here that gets a recommended action, since a drop is itself the trigger for doing something. Each event fires once per real occurrence — never re-send the same milestone.
